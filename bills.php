            ...
            if ($action === 'add_bill') {
                ...
                $db->execute(
                    "INSERT INTO bills (user_id, category_id, name, amount, due_date, is_recurring, recurring_period, auto_pay, priority, threshold_warning, notes) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [$user_id, $category_id, $name, $amount, $due_date, $is_recurring, $recurring_period, $auto_pay, $priority, $threshold_warning, $notes]
                );
...
if ($status_filter && in_array($status_filter, ['pending', 'paid', 'overdue'])) {
...
<body class="<?= $darkMode ? 'dark bg-gray-900' : 'bg-gray-50' ?> font-sans">
        ...
        <div class="flex flex-col h-full">
            ...
            <nav class="flex-1 p-4 space-y-2">
                ...
                <a href="bills.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg gradient-primary text-white">
                    ...
                    <span>Bills</span>
                    <?php if ($stats['pending_count'] + $stats['overdue_count'] > 0): ?>
                        <span class="bg-red-500 text-white text-xs px-2 py-1 rounded-full notification-badge"><?= $stats['pending_count'] + $stats['overdue_count'] ?></span>
                    <?php endif; ?>
                </a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <?php else: ?>
                    <?php foreach ($bills as $bill): ?>
                        ...
                        $statusClass = $bill['status'] === 'overdue' ? 'bill-overdue' : 
                                      ($bill['status'] === 'pending' ? 'bill-pending' : 'bill-paid');
                        ?>
                        <div class="<?= $darkMode ? 'bg-gray-800' : 'bg-white' ?> rounded-xl p-6 shadow-lg bill-card <?= $statusClass ?>">
                               ...
                               <div class="flex justify-between items-center">
                                   ...
                                   ?>">
                                       <i class="fas fa-<?= 
                                           ...
                                           ($bill['status'] === 'pending' ? 'clock' : 'check-circle')
                                       ?> mr-1"></i>
                                       <?= ucfirst($bill['status']) ?>
                                   </span>
                               </div>
                           ...
                           <?php if ($bill['status'] === 'pending' || $bill['status'] === 'overdue'): ?>
                               <div class="mt-4 pt-4 border-t <?= $darkMode ? 'border-gray-700' : 'border-gray-200' ?>">
                                   <button onclick="payBillNow(<?= $bill['id'] ?>)" class="w-full gradient-success hover:opacity-90 text-white py-2 px-4 rounded-lg transition-all transform hover:scale-105">
   ...
   <script>
       ...
       document.getElementById('bill-due-date').min = new Date().toISOString().split('T')[0];
       
       // Enable/disable recurring period based on recurring checkbox
       document.getElementById('bill-is-recurring').addEventListener('change', function() {
       ...
       // Initialize recurring period state
       document.addEventListener('DOMContentLoaded', function() {
           const isRecurringCheckbox = document.getElementById('bill-is-recurring');
           const recurringPeriod = document.getElementById('bill-recurring-period');
